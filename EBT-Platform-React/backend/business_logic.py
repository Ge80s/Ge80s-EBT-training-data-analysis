...
def load_single_file(file_path: str, source_label: str = None) -> Dict:
    ...
    try:
        ...
        # OB 列检测
        ob_cols_list = []
        def _parse_ob_metadata(col_name: str):
            """兼容 2025 旧格式 (PRO_OB_1) 与 2026 新格式 (PRO OB 1 / KNO OB.2 / SAW-OB3)。"""
            col_text = str(col_name)
            if not col_text:
                return {'competency': 'Unknown', 'ob_item': 'OB'}

            normalized = col_text.replace('【', '').replace('】', '').strip()
            normalized = re.sub(r'\s+', ' ', normalized)
            normalized = normalized.replace('：', ':')

            # 直接从列名提取能力维度 + OB 编号
            match = re.search(r'([A-Za-z]{3,6})\s*[-_\s]*OB\s*[-_\s]*0?([1-9][0-9]?)', normalized, re.IGNORECASE)
            if match:
                competency_code = match.group(1).upper()
                ob_number = match.group(2)
                return {'competency': comp_map.get(competency_code, competency_code), 'ob_item': f'OB{ob_number}'}

            match = re.search(r'([A-Za-z]{3,6})\s*[-_\s]*0?([1-9][0-9]?)', normalized, re.IGNORECASE)
            if match and 'OB' in normalized.upper():
                competency_code = match.group(1).upper()
                ob_number = match.group(2)
                return {'competency': comp_map.get(competency_code, competency_code), 'ob_item': f'OB{ob_number}'}

            if 'OB' in normalized.upper():
                # 2026 更常见的写法：KNO OB.2 / PRO OB 1
                match = re.search(r'OB\s*[-_.]?\s*0?([1-9][0-9]?)', normalized, re.IGNORECASE)
                if match:
                    ob_number = match.group(1)
                    competency = 'Unknown'
                    for prefix in ['KNO','PRO','FPA','FPM','COM','LTW','SAW','WLM','PSD']:
                        if normalized.upper().startswith(prefix):
                            competency = prefix
                            break
                    return {'competency': comp_map.get(competency, competency), 'ob_item': f'OB{ob_number}'}

            # 兜底：按下划线/符号分段解析
            for delim in ['_', ' ', ':', '-', '.']:
                parts = [p for p in re.split(rf'[{re.escape(delim)}]+', normalized) if p]
                if len(parts) >= 2 and 'OB' in ''.join(parts).upper():
                    for idx, part in enumerate(parts):
                        if 'OB' in part.upper():
                            ob_match = re.search(r'0?([1-9][0-9]?)', part, re.IGNORECASE)
                            if ob_match:
                                competency = parts[0] if parts[0].upper() in comp_map else 'Unknown'
                                return {'competency': comp_map.get(competency, competency), 'ob_item': f'OB{ob_match.group(1)}'}
        ...
        for col in df.columns:
            col_text = str(col)
            if 'OB' in col_text.upper() and '评语' not in col_text and '评论' not in col_text:
                parsed = _parse_ob_metadata(col)
                if parsed['competency'] != 'Unknown' or 'OB' in col_text.upper():
                    ob_cols_list.append({
                        'col_name': col,
                        'competency': parsed['competency'],
                        'ob_item': parsed['ob_item'],
                    })
